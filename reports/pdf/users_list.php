...
if (in_array($_SESSION['session_fleet']['level'], $levels))
{
	if (isset($_SESSION['data_report']))
	{
		...
		// Cabecera de página
			...
			function Header() {
				...
				$this->Image('../../dist/img/logo.png', 10, 10, 40); //imagen(archivo, png/jpg || x,y,tamaño)
				$this->setXY(60, 15);
				$this->Cell(130, 13, utf8_decode('Usuarios con autorización de licencia'), 0, 1, 'C', 0);
				$this->Ln(5);
			}
			...
			function CheckPageBreak($h, $setX) {
				...
				if ($this->GetY() + $h > $this->PageBreakTrigger) {
					...
					else if (FILE_SIDE == 'l')
					{
						...
						$this->Cell(65, 8, 'Cargo', 1, 0, 'C', 0);
						$this->Cell(50, 8, 'Region', 1, 0, 'C', 0);
						$this->Cell(30, 8, 'Estado', 1, 1, 'C', 0);
						$this->SetFont('Arial', '', 9);
					}
				}
		...
		else if (FILE_SIDE == 'l')
		{
			...
			$pdf->Cell(50, 8, 'Email', 1, 0, 'C', 0);
			$pdf->Cell(65, 8, 'Cargo', 1, 0, 'C', 0);
			$pdf->Cell(50, 8, 'Region', 1, 0, 'C', 0);
			$pdf->Cell(30, 8, 'Estado', 1, 1, 'C', 0);
			// -------TERMINA----ENCABEZADO------------------
		...
		if ($data)
		{
			...
			{
				$pdf->Row(array($i + 1, utf8_decode($data['name'][$i]), utf8_decode($data['email'][$i]), utf8_decode($data['position'][$i]), utf8_decode($data['region'][$i]), utf8_decode($data['approval_name'][$i])), 8);
			}
		}
		else
		{
		...
		$name = "Reporte de usuarios al {$fecha}.pdf";
